...
run_graph = function(iters ){
  ...
  repeat {
    ...
    for(i in 1:200){
      ...
      rw <- c(samp_type,actnodes,actedges,sampnodes,sampedges,samplingrate,dim(real_comm_size),dim(sampled_comm_sizeg1)  ,actualmodularity,sampledmodularityg1,Lcommsize_act1,Lcommsize_samp1,Lcommsize_act2,Lcommsize_samp2,Lcommsize_act3,Lcommsize_samp3,clustcoeff_act,clustcoeff_samp,diameter_act,diameter_samp,ksval$statistic,ksval$p.value,ks_cc$statistic,ks_cc$p.value,ks_pg$statistic,ks_pg$p.value)
      dt <- rbind(dt,rw)
